...
const testimonials = [
  {
    ...
    quote:
      "My oat milk latte is ready before I even reach the counter. Same order, same great taste, every single time.",
  },
  {
    name: "Jerome Villaruel",
    role: "Dipolog City local",
    quote:
      "Best lunch spot near the plaza. Quick pickup line and the food photos on the app actually match what you get.",
  },
  {
    name: "Andrea Cruz",
    role: "Weekend regular",
    quote:
      "Cozy enough to stay a while, fast enough when I just need to grab and go. Streetside gets both right.",
  },
];
          ...
          <div className="border-t border-white/70 bg-white/95 px-4 py-4 backdrop-blur-xl dark:border-white/10 dark:bg-zinc-950/95 md:hidden">
            <nav className="flex flex-col gap-1 text-sm font-medium text-zinc-600 dark:text-zinc-300">
              <a
                ...
                className="rounded-xl px-3 py-2.5 transition-colors hover:bg-zinc-100 hover:text-zinc-950 dark:hover:bg-zinc-900 dark:hover:text-white"
              >
                Menu
              </a>
              <a
            ...
            <Button className="mt-3 w-full gap-2" size="lg" onClick={goToOrder}>
              Order Now
              <ArrowRight className="h-4 w-4" />
            </Button>
          </div>
      ...
      <main id="top">
        ...
        <section className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-20">
          <div className="flex flex-col justify-center">
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-600 dark:text-zinc-300">
              A calm, modern café experience for pickup orders, quick lunch
              runs, and slow afternoons. Clean design, fast ordering, and menu
              items that look as good as they taste.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <Button size="lg" className="gap-2" onClick={goToOrder}>
                Order Now
                <ArrowRight className="h-4 w-4" />
              </Button>
                      ...
                      <div className="max-w-xl text-white">
                        ...
                        <h2 className="mt-2 font-heading text-3xl font-semibold tracking-tight sm:text-4xl">
                          {activeSlideItem
                            ...
                            : "Smooth transitions, fresh photos, menu that feels alive."}
                        </h2>
                        <p className="mt-3 max-w-lg text-sm leading-6 text-white/75 sm:text-base">
                          {activeSlideItem?.description ||
                            "A rotating showcase of best sellers and new drinks."}
                        </p>
                        {activeSlideItem ? (
                ...
                {loading ? (
                  <div className="mt-6 space-y-4">
                    {Array.from({ length: 3 }).map((_, index) => (
                      <div
                        key={index}
                        className="h-24 animate-pulse rounded-3xl bg-zinc-100 dark:bg-zinc-800"
                      />
                    ))}
                  </div>
                ) : (
        ...
        >
          <LandingSection
            ...
            description="The best-selling pick from each menu category, so you can see the range of the café at a glance."
            className="mb-8"
          />
          ...
          {loading ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="h-72 animate-pulse rounded-3xl bg-zinc-100 dark:bg-zinc-900"
                />
              ))}
            </div>
          ) : topSellersByCategory.length === 0 ? (
                  ...
                  >
                    ...
                    <CardContent className="p-5">
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        ...
                        <Button size="sm" variant="outline" onClick={goToOrder}>
                          Order
                        </Button>
                      </div>
                    </CardContent>
        ...
        {/* ========== ONLINE ORDER (cart + name + place order) ========== */}
        <OnlineOrderSection />
        ...
        >
          <LandingSection
            ...
            description="The layout is structured to feel premium, scan quickly, and leave room for future brand, loyalty, or promotion modules."
            className="mb-6"
          />
        ...
        >
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {testimonials.map((t) => (
              <Card
                key={t.name}
                className="border-white/80 bg-white/75 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-zinc-900/70"
              >
        ...
        <section
          id="visit"
          className="border-t border-white/80 bg-white/60 dark:border-white/10 dark:bg-zinc-950/50"
        >
